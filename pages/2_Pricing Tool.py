...
with st.sidebar:
    ...
    if gym_details is not None:
        market = st.selectbox("Market:", gym_details['market'].unique())
    else:
        market = st.selectbox("Market:", ["NYC", "LA", "Chicago", "Miami"])

    product_type = st.selectbox("Product Type:", ['standard', 'premium'])
    tier = st.selectbox("Tier:", ['High-Tier', 'Mid-Tier', 'Low-Tier'])
